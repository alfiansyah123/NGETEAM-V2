...
// .wrangler/tmp/pages-2BYRNK/functionsWorker-0.22460486581799366.mjs
...
async function onRequestPost(context) {
  ...
  try {
    const { domain: domain2, cfToken, cfAccountId } = await context.request.json();
    if (!domain2 || !cfToken || !cfAccountId) {
    ...
    const domainsToLink = [domain2, `*.${domain2}`];
    const results = [];
    let hasError = false;
    for (const domainName of domainsToLink) {
      ...
      if (!cfData.success) {
        ...
        const alreadyExists = errors.some(
          (e) => e.code === 8e6 || e.message?.toLowerCase().includes("already added") || e.message?.toLowerCase().includes("already exists") || e.message?.toLowerCase().includes("duplicate")
        );
        if (alreadyExists) {
    ...
    const rootSuccess = results.find((r) => r.domain === domain2 && (r.message || !r.error));
    return new Response(JSON.stringify({
      success: !!rootSuccess,
      results,
    ...
    if (!cfData.success) {
      ...
      if (alreadyExists) {
        const listResponse = await fetch(
          `https://api.cloudflare.com/client/v4/zones?name=${domain2}&account.id=${cfAccountId}`,
          {
            headers: {
              "Authorization": `Bearer ${cfToken}`,
              "Content-Type": "application/json"
...
async function onRequestPost4(context) {
  ...
  try {
    const { zoneId, domain: domain2, cfToken } = await context.request.json();
    if (!zoneId || !domain2 || !cfToken) {
      return new Response(JSON.stringify({
    ...
    for (const record of records) {
      const checkResponse = await fetch(
        `https://api.cloudflare.com/client/v4/zones/${zoneId}/dns_records?type=${record.type}&name=${record.name === "@" ? domain2 : record.name + "." + domain2}`,
        {
          headers: {
            "Authorization": `Bearer ${cfToken}`,
            "Content-Type": "application/json"
...
async function onRequestPost5(context) {
  ...
  try {
    ...
    const pagesTarget = context.env.PAGES_DOMAIN || "ngeteam-v2.pages.dev";
    const workerName = `proxy-${domain2.replace(/\./g, "-")}`;
    const scriptContent = `
...
async function handleRequest(request) {
  ...
  try {
    ...
    const routes2 = [`${domain2}/*`, `*.${domain2}/*`];
    const routeResults = [];
    const zoneRes = await fetch(
      `https://api.cloudflare.com/client/v4/zones?name=${domain2}`,
      {
        headers: {
          "Authorization": `Bearer ${cfToken}`,
          "Content-Type": "application/json"
    ...
    if (!zoneData.success || zoneData.result.length === 0) {
      throw new Error("Could not find Zone ID for domain: " + domain2);
    }
    const zoneId = zoneData.result[0].id;
    for (const pattern of routes2) {
  ...
  try {
    ...
    for (const line of lines) {
      if (line.trim().startsWith("INSERT INTO")) {
        stmts.push(db.prepare(line.trim()));
      }
    }
    let successCount = 0;
    ...
    return new Response("Import successful! Rows inserted: " + successCount, { status: 200 });
  } catch (err) {
...
async function onRequest(context) {
  ...
  try {
    ...
    const domain2 = reqData.domain || reqData.url;
    if (!domain2) {
      return new Response(JSON.stringify({ error: "Domain is required" }), { status: 400, headers });
    }
    const cleanDomain = (domain2 || "").replace(/^https?:\/\//, "").replace(/\/$/, "").trim();
    if (!cleanDomain) {
...
async function onRequestGet5(context) {
  ...
  try {
    ...
    return new Response(JSON.stringify({
      ...
      debug: link
      // Kita intip semua isinya di sini!
    }), { status: 200, headers });
  } catch (error) {
    console.error("Database error:", error);
  ...
  try {
    ...
    const mergedTeam = team.map((member) => {
      ...
      const linkData = (links || []).find(
        (l) => (l.user_id || "").toLowerCase() === memberId || (l.slug || "").toLowerCase() === memberId
      );
      return {
        ...member,
...
async function onRequestPost13(context) {
  ...
  try {
    ...
    let { slug, original_url, domain_url, title: title2, description, image_url, user_id, url_trafee, routing_mode } = body;
    ...
    if (!title2 && !image_url) {
      try {
        ...
        const getOg = /* @__PURE__ */ __name2((prop) => {
          const m = rawHtml.match(new RegExp(`<meta[^>]+property=["']` + prop + `["'][^>]+content=["']([^"']+)["']`, "i")) || rawHtml.match(new RegExp(`<meta[^>]+content=["']([^"']+)["'][^>]+property=["']` + prop + `["']`, "i"));
          return m ? m[1] : "";
        }, "getOg");
        if (!title2) {
          title2 = getOg("og:title") || (rawHtml.match(/<title[^>]*>([^<]+)<\/title>/i) || [])[1] || "";
        }
        if (!description) {
    ...
    const domain2 = await db.prepare(`
...
var version22 = "2.97.0";
var DEFAULT_HEADERS = { "X-Client-Info": `storage-js/${version22}` };
...
async function onRequestPost18({ request, env: env2 }) {
  const supabaseUrl = env2.SUPABASE_URL || env2.VITE_SUPABASE_URL;
  const supabaseKey = env2.SUPABASE_KEY || env2.VITE_SUPABASE_KEY;
  ...
  try {
    ...
    let { data, error } = await supabase.storage.from("images").upload(fileName, file, {
      cacheControl: "3600",
      upsert: false
    });
...
async function onRequestPost19({ request, env: env2 }) {
  const headers = {
    "Access-Control-Allow-Origin": "*",
    "Content-Type": "application/json"
  };
...
__name2(getSecurityHeaders, "getSecurityHeaders");
async function recordClick(db, link, visitorData, env2, clickId, networkName) {
...
<meta name="twitter:card" content="summary_large_image">
  ...
  let clickId = null;
  const extractViaRegex = /* @__PURE__ */ __name2((urlStr) => {
    if (!urlStr) return null;
    const match2 = urlStr.match(/[?&](click_id|clickid|subid|track)=([^&\s]+)/i);
...
// .wrangler/tmp/bundle-mQ7S9B/middleware-insertion-facade.js
...
// .wrangler/tmp/bundle-mQ7S9B/middleware-loader.entry.ts
...
//# sourceMappingURL=functionsWorker-0.22460486581799366.js.map

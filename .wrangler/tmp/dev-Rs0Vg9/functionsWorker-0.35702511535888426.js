...
// .wrangler/tmp/pages-o1iu7W/functionsWorker-0.35702511535888426.mjs
...
async function onRequestGet6(context) {
  ...
  try {
    ...
    const { data: link, error: linkError } = await supabase.from("links").select("original_url").eq("slug", team.user_id).maybeSingle();
    return new Response(JSON.stringify({
      success: true,
      data: {
        name: team.name,
...
async function onRequestPost16(context) {
  ...
  try {
    ...
    if (teamError) throw teamError;
    if (old_user_id) {
      const { error: primaryError } = await supabase.from("links").update({
        slug: user_id,
        user_id,
        original_url: target_url
      }).eq("slug", old_user_id);
      if (primaryError) console.warn("Primary link update failed:", primaryError);
      const { error: linksError } = await supabase.from("links").update({
        original_url: target_url,
        user_id
...
// .wrangler/tmp/bundle-axKF45/middleware-insertion-facade.js
...
// .wrangler/tmp/bundle-axKF45/middleware-loader.entry.ts
...
//# sourceMappingURL=functionsWorker-0.35702511535888426.js.map

...
// .wrangler/tmp/bundle-rVIiy1/checked-fetch.js
...
// .wrangler/tmp/pages-6WjURc/functionsWorker-0.037325213867609564.mjs
...
async function onRequestPost9(context) {
  ...
  try {
    ...
    for (let i = 0; i < statements.length; i += batchSize) {
      const batch = statements.slice(i, i + batchSize).map((s) => db.prepare(s));
      await db.batch(batch);
      successCount += batch.length;
      console.log(`Imported ${successCount}/${statements.length}`);
    }
    return new Response("Import successful! Statements executed: " + successCount, { status: 200 });
  } catch (err) {
...
async function onRequestPost19({ request, env }) {
  ...
  try {
    const { urls: urls22 } = await request.json();
    if (!urls22 || !Array.isArray(urls22) || urls22.length === 0) {
      return new Response(JSON.stringify({ success: false, error: "No URLs provided" }), { status: 400, headers });
    }
    const toWarm = urls22.slice(0, 10);
...
async function onRequest5(context) {
  const db = context.env.DB;
  const tables = ["domains", "team", "links", "clicks", "settings", "daily_reports", "conversions", "users"];
  try {
    for (const table of tables) {
      await db.prepare(`DROP TABLE IF EXISTS ${table}`).run();
    }
    return new Response("All tables dropped successfully!", { status: 200 });
  } catch (err) {
...
// .wrangler/tmp/bundle-rVIiy1/middleware-insertion-facade.js
...
// .wrangler/tmp/bundle-rVIiy1/middleware-loader.entry.ts
...
//# sourceMappingURL=functionsWorker-0.037325213867609564.js.map

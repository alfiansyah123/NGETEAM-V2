...
async function onRequestPost7(context) {
  ...
  try {
    ...
    const { data, error } = await supabase.from("team").insert({ name, user_id, password }).select().single();
    if (error) {
      if (error.code === "23505") {
...
async function onRequestGet6(context) {
  ...
  try {
    const { data: team, error: teamError } = await supabase.from("team").select("*").ilike("user_id", userId).maybeSingle();
    if (teamError) throw teamError;
    if (!team) {
      return new Response(JSON.stringify({ error: "Team member not found" }), { status: 404, headers });
    }
    const { data: link, error: linkError } = await supabase.from("links").select("original_url").eq("user_id", team.user_id).maybeSingle();
    return new Response(JSON.stringify({
      success: true,
      data: {
        name: team.name,
...
async function onRequestGet7(context) {
  ...
  try {
    ...
    let links = [];
    if (userIds.length > 0) {
      const { data: linksData, error: linksError } = await supabase.from("links").select("slug, original_url, user_id").in("user_id", userIds);
      if (!linksError) {
        links = linksData || [];
      } else {
...
async function onRequestPost10(context) {
  ...
  try {
    ...
    const { data: teamMember, error: teamError } = await supabase.from("team").select("*").eq("user_id", username).eq("password", password).maybeSingle();
    if (teamMember) {
      const token = generateToken(username);
      return new Response(JSON.stringify({
        success: true,
...
async function onRequestPost11(context) {
  ...
  try {
    ...
    const { data: domainList, error: domainError } = await supabase.from("domains").select("id").ilike("url", domain_url).eq("active", true);
...
async function onRequestPost13(context) {
  ...
  try {
    ...
    const { error: teamError } = await supabase.from("team").update({ name, user_id, password }).eq("id", id);
    if (teamError) throw teamError;
    if (old_user_id) {
...
async function recordClick(supabase, link, request) {
  ...
  let clickId = requestUrl.searchParams.get("click_id") || requestUrl.searchParams.get("clickid") || requestUrl.searchParams.get("subid");
  ...
  const getBestIP = /* @__PURE__ */ __name(() => {
    ...
    const isWorkerProxy = /* @__PURE__ */ __name((ip2) => ip2 && (ip2 === "2a06:98c0:3600::103" || ip2.startsWith("2a06:98c0")), "isWorkerProxy");
    if (xff) {
...
async function onRequest2(context) {
  ...
  if (lookUpSlug) {
    const { data: teamMember } = await supabase.from("team").select("id").eq("user_id", lookUpSlug).maybeSingle();
    if (teamMember) {
      return context.next();
    }
  }
...
// ../.wrangler/tmp/pages-kVQu56/functionsRoutes-0.41049075424490455.mjs
var routes = [
  {
...
// ../.wrangler/tmp/bundle-0SvYL0/middleware-insertion-facade.js
...
// ../.wrangler/tmp/bundle-0SvYL0/middleware-loader.entry.ts
...
//# sourceMappingURL=functionsWorker-0.06695356364234484.mjs.map
